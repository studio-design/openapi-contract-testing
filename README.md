...
- **`format`** (validated by opis Draft 06+): `date`, `time`, `date-time`, `duration`, `uri`, `uri-reference`, `uri-template`, `iri`, `iri-reference`, `regex`, `ipv4`, `ipv6`, `hostname`, `idn-hostname`, `uuid`, `email`, `idn-email`, `json-pointer`, `relative-json-pointer`. Unknown values (e.g. `format: emial` typo for `email`) emit a one-shot `E_USER_WARNING` per format value, since opis silently accepts any string for unrecognised formats.
- **Advisory `format`** (deliberately not enforced, no warning): `int32`, `int64`, `float`, `double`, `byte`, `binary`, `password`. Treated as documentation hints per OAS conventions.
