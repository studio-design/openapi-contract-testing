...
- **Cascading `additionalProperties: false` errors** are stripped automatically. opis's `PropertiesKeyword` early-returns on any sub-property failure and never propagates `$checked` to the validation context, so the follow-on `additionalProperties: false` keyword reports every property the data carries — including ones explicitly declared in the schema's `properties` — as "additional". The validator detects this pattern (the listed names match declared properties at that path) and drops the cascade so a single failure shows as one error, not a paired pseudo-error naming declared properties as not-allowed. Genuine additional properties still surface; mixed cases keep only the real extras in the message. The dedup is structural (schema-introspection-based, not message-string-based for the property list) so a `oneOf` / `allOf` path that can't be resolved against the top-level schema falls back to keeping the original message untouched.
